...
class Card:
    ...
    def __init__(self, card_string):
        self.value, self.suit = card_string[0], card_string[1]
        self.value = suit_value_dict[self.value]
...
def parseCards(playerDict):
    for key, value in playerDict.items():
        playerDict[key][0] = Card(value[0])
        playerDict[key][1] = Card(value[1])

    return playerDict

def parseBoard(board):
    for x, card in enumerate(board):
        board[x] = Card(card)

    return board

# Returns deck of cards with all hole cards and board cards removed
def generate_deck(playerDict, board):
    deck = []
...
def detect_hand(playerCards, given_board, suit_histogram,
...
def find_winner(generate_boards, deck, playerDict, num,
    ...
    for remaining_board in generate_boards(deck, num, board_length):
        ...
        for key, value in playerDict.items():
            result_list[int(key)-1] = detect_hand(value, board, suit_histogram,
                                             histogram, max_suit)
